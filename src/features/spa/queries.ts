...
export async function getHotelSpaSettings(hotelId: string, supabase: SupabaseClient): Promise<HotelSpaSettings | null> {
  const { data, error } = await supabase
    ...
    .select("id, hotel_id, enabled, opens_at, closes_at, slot_duration_minutes, capacity_per_slot, price_per_person, allow_non_residents, advance_booking_days, min_notice_hours, created_at, updated_at")
    .eq("hotel_id", hotelId)
...
export async function listSpaBookings(hotelId: string, supabase: SupabaseClient): Promise<SpaBooking[]> {
  const { data, error } = await supabase
    .from("spa_bookings")
    .select(
      "id, hotel_id, conversation_id, guest_name, guest_phone_e164, party_size, is_non_resident, notes, booking_date, slot_start, slot_end, price_per_person_snapshot, status, cancelled_by, cancelled_at, owner_notification_status, owner_notified_at, created_at, updated_at"
    )
    .eq("hotel_id", hotelId)
